...
client.on("interactionCreate", async (interaction) => {
  if (interaction.isModalSubmit()) {
    if ((interaction as any).customId === "eventModal") {
     ...
     const message:any =  await client.rest.post(Routes.channelMessages(String(process.env.CHANNEL_ID)), // copied channel id
       {
        body: {
      ...
      await client.rest.post(Routes.threads(String(process.env.CHANNEL_ID),message.id), // copied channel id
       {
        body: {
          name: eventName
        },
